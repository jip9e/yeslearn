  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-[640px] mx-auto pl-14 md:pl-8">
      ...
      <div className="flex flex-col gap-6">
        {/* Space Selector */}
        <div>
          <label className="block text-[13px] font-medium mb-2">Space</label>
          {spaces.length === 0 ? (
            <div className="p-4 rounded-xl border border-[#e5e5e5] bg-[#fafafa] text-center">
              <p className="text-[13px] text-[#999] mb-2">No spaces yet</p>
              <Link href="/space/new" className="text-[13px] text-black font-medium hover:opacity-70">
        ...
        <div>
          <label className="block text-[13px] font-medium mb-2">Content Type</label>
          <div className="grid grid-cols-2 gap-3">
            {CONTENT_TYPES.map((type) => (
              ...
              >
                ...
                <p className="text-[11px] text-[#999] mt-0.5">{type.description}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="block text-[13px] font-medium mb-2">
            Name <span className="text-[#ccc]">(auto-generated if empty)</span>
          </label>
          <input
            type="text"
            value={name}
            ...
            className="w-full px-4 py-3 rounded-xl border border-[#e5e5e5] bg-white text-[14px] focus:outline-none focus:ring-2 focus:ring-black/10 placeholder:text-[#ccc]"
          />
        </div>
        ...
        {selectedType === "pdf" && (
          <div>
            <label className="block text-[13px] font-medium mb-2">PDF File</label>
            <input
              type="file"
              ref={fileInputRef}
              accept=".pdf"
            ...
            {file ? (
              ...
              <button
                ...
                className="w-full p-8 rounded-xl border-2 border-dashed border-[#e0e0e0] flex flex-col items-center gap-2 hover:border-[#bbb] hover:bg-[#fafafa] transition-all"
              >
                <Upload size={24} className="text-[#ccc]" />
                <p className="text-[13px] text-[#999]">Click to select a PDF file</p>
                <p className="text-[11px] text-[#ccc]">Max 50MB</p>
              </button>
            )}
          </div>
        )}
        ...
        {selectedType === "text" && (
          <div>
            <label className="block text-[13px] font-medium mb-2">Text Content</label>
            <textarea
              value={textContent}
              onChange={(e) => setTextContent(e.target.value)}
              placeholder="Paste or type your notes, text content, etc."
              rows={8}
              className="w-full px-4 py-3 rounded-xl border border-[#e5e5e5] bg-white text-[14px] focus:outline-none focus:ring-2 focus:ring-black/10 resize-none placeholder:text-[#ccc]"
            />
          </div>
        )}
        ...
        <button
          ...
          className="w-full py-3 rounded-xl bg-black text-white text-[14px] font-medium hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {submitting ? (
            <>
